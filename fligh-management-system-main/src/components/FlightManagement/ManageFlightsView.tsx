...
export const ManageFlightsView: React.FC<ManageFlightsViewProps> = ({
  ...
  // Filter state â€” defaults to the current UTC day so the list opens on today.
  ...
  // Naming details for the exported report. The worksheet/tab name is the
  // displayed (filtered) date; a range freezes the "From -> To" span when active.
  const exportDateLabel = isFilterActive ? filterFromDate : todayParts.date;
  const exportRangeLabel = isFilterActive
    ? `${filterFromDate} ${filterFromTime || '00:00'} â†’ ${filterToDate} ${filterToTime || '23:59'} UTC`
    : 'All dates';

  // Export rows always in chronological STA order (00:00 UTC earliest to 23:59 UTC
  // latest), independent of whatever column the on-screen table is currently sorted
  // by. `filteredFlights` alone is raw DB order and `sortedFlights` follows the
  // user's column sort; neither is guaranteed, so sort a copy by STA ascending.
  const exportedFlights = useMemo(
    () => [...filteredFlights].sort(
      (a, b) => new Date(a.staUtc).getTime() - new Date(b.staUtc).getTime()
    ),
    [filteredFlights]
  );

  const handleExportExcel = () => {
    if (exportedFlights.length === 0) {
      showToast('No flights to export for the current filter.', 'warning');
      return;
    }
    exportFlightsToExcel(exportedFlights, exportDateLabel, exportRangeLabel);
    showToast(`Exported ${exportedFlights.length} flights to Excel.`, 'success');
  };
  ...
  const handleExportPdf = () => {
    ...
    try {
      exportFlightsToPdf(exportedFlights, exportDateLabel, exportRangeLabel);
      showToast('PDF export opened in a new tab.', 'success');
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Failed to open PDF export.', 'error');
    }
  };
      ...
      <div className="glass-card p-3 rounded-2xl shadow-xl dark:shadow-2xl flex flex-wrap items-center justify-between gap-2.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            title="Export the filtered flights to a styled Excel workbook"
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => {
                ...
                setIsFilterActive(true);
                showToast('Showing todayâ€™s flights (UTC).', 'info');
              }}
          ...
          <div className="text-slate-500 dark:text-slate-400">
            {isFilterActive ? (
              <span className="text-sky-700 dark:text-sky-300 font-medium">
                ًں”چ Filtered: Showing flights from <span className="font-mono text-slate-800 dark:text-slate-200">{formatUtcDateTime(toFilterUtc(filterFromDate, filterFromTime))}</span> to{' '}
      ...
      {groundHandlingAlerts.total > 0 && (
        ...
        >
          ...
          <span className="text-slate-600 dark:text-slate-300">
            Warning fires when an arrived flight reaches 10 minutes before its scheduled ground handling time (STD âˆ’ STA).
          </span>
        </div>
      )}
            ...
            <div className="flex items-center justify-between border-b border-slate-200 bg-gradient-to-r from-sky-900 to-slate-800 px-5 py-4 text-white dark:border-white/10">
              <div>
                ...
                <p className="text-xs text-sky-100">
                  {selectedFlight.inboundFlightNumber} / {selectedFlight.outboundFlightNumber} آ· {selectedFlight.flightStatus}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="p-3.5 glass-card-sub rounded-xl border border-slate-200 dark:border-white/10 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  {txtVia && txtOrigin && txtDestination && txtFinalDestination && txtOrigin === txtFinalDestination && (
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-500/40 flex items-center gap-1">
                      ًں”؛ Active Triangle Flight Rotation (3 Sectors)
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  >
                    <span>ًں”؛ Setup Triangle (LHR-CAI-HRG-LHR)</span>
                  </button>
